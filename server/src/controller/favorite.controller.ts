...
export function createFavoriteRouter(service: FavoriteService) {
  const router = new Router();

  router.post('/toggle', requireAuth(), async (ctx: any) => {
    const { note_id } = ctx.request.body as any;
    if (!note_id) { ctx.status = 400; ctx.body = { code: 400, message: 'note_id 不能为空' }; return; }
    try {
      const result = await service.toggle(getUserId(ctx), Number(note_id));
