...
export default function HomestaysPage() {
  ...
  const openEdit = (record) => {
    ...
    form.setFieldsValue({
      ...
      style_tags: record.styleTags, facility_tags: record.facilityTags, main_image: record.mainImage,
      intro: record.intro, status: record.status,
    });
    setModalOpen(true);
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editing) {
        await request.put(`/api/homestay/update/${editing.id}`, values);
        message.success('更新成功');
      } else {
        await request.post('/api/homestay/create', values);
  ...
  const handleDelete = async (id) => {
    try {
      await request.delete(`/api/homestay/delete/${id}`);
  ...
  const columns = [
    ...
    { title: '风格标签', dataIndex: 'styleTags', width: 150, ellipsis: true, render: (v) => v || '-' },
  ...
  return (
    <div>
      ...
      <Modal title={editing ? '编辑民宿' : '新增民宿'} open={modalOpen} onOk={handleSubmit} onCancel={() => setModalOpen(false)} destroyOnClose width={640}>
        <Form form={form} layout="vertical">
          <Form.Item name="name" label="民宿名称" rules={[{ required: true, message: '请输入名称' }]}>
            <Input placeholder="如：云栖山居" />
          </Form.Item>
          <Form.Item name="address" label="地址">
            <Input placeholder="详细地址" />
          </Form.Item>
          <Form.Item name="latitude" label="纬度">
            <InputNumber style={{ width: '100%' }} placeholder="如：26.123456" />
          </Form.Item>
          <Form.Item name="longitude" label="经度">
            <InputNumber style={{ width: '100%' }} placeholder="如：108.123456" />
          </Form.Item>
          <Form.Item name="style_tags" label="风格标签（逗号分隔）">
            <Input placeholder="如：田园风,禅意,亲子" />
          </Form.Item>
