/**
 * 轮播图管理页面
 * 展示平台首页轮播图列表，支持搜索、新增、编辑、删除操作
 * 轮播图包含图片、标题、跳转链接、排序和上下架状态
 */
import { useEffect, useState } from 'react';
import { Table, Button, Space, Input, Modal, Form, InputNumber, Select, message, Popconfirm, Tag, Image } from 'antd';
...
export default function CarouselListPage() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const res: any = await request.get('/carousels/list', { params: { page, pageSize, keyword } });
  ...
  const handleSubmit = async () => {
    ...
    values.image_url = imageUrl; // 使用上传的图片 URL
    if (!values.image_url) {
      message.error('请上传图片');
      return;
    }
    try {
      if (editing) {
        const res: any = await request.put(`/carousels/update/${editing.id}`, values);
        if (res.code === 200) { message.success('更新成功'); setModalOpen(false); loadData(); }
        else message.error(res.message);
      } else {
        const res: any = await request.post('/carousels/create', values);
  ...
  const handleDelete = async (id: number) => {
    const res: any = await request.delete(`/carousels/delete/${id}`);
  ...
  const columns = [
    ...
    { title: '状态', dataIndex: 'status', render: (v: number) => v === 1 ? <Tag color="green">上架</Tag> : <Tag color="default">下架</Tag> },
    {
  ...
  return (
    <div>
      <h2 style={{ marginBottom: 'var(--spacing-md)', fontSize: 'var(--text-h2)', fontFamily: 'var(--font-family-heading)', fontWeight: 'var(--weight-bold)', color: 'var(--color-text-primary)' }}>轮播图管理</h2>
      <Space style={{ marginBottom: 16 }}>
        ...
        <Button type="primary" icon={<PlusOutlined />} onClick={() => openModal()}>新增轮播图</Button>
      </Space>
      ...
      <Modal title={editing ? '编辑轮播图' : '新增轮播图'} open={modalOpen} onOk={handleSubmit} onCancel={() => setModalOpen(false)} destroyOnClose width={600}>
        <Form form={form} layout="vertical">
          ...
          <Form.Item name="sort_order" label="排序" initialValue={0}><InputNumber min={0} style={{ width: '100%' }} placeholder="数值越大越靠前" /></Form.Item>
