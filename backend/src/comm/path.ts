 ...
 */
const getKeys = () => {
  const jsConfigFile = path.join(__dirname, '../config/config.default.js');
  const tsConfigFile = path.join(__dirname, '../config/config.default.ts');
  const configFile = fs.existsSync(jsConfigFile) ? jsConfigFile : tsConfigFile;
